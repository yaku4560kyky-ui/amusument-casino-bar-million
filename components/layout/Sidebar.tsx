...
export default function Sidebar({ profile }: { profile: Profile }) {
  ...
  const navLinks = profile.role === 'admin' ? [...links, { href: '/admin', label: '管理者', icon: Shield }] : links
  ...
  return (
    <aside className="fixed left-0 top-0 z-20 flex h-full w-64 flex-col border-r border-border bg-card text-card-foreground shadow-xl max-md:static max-md:h-auto max-md:w-full">
      <div className="border-b border-border p-5">
        ...
        <p className="text-xs text-muted-foreground">{profile.role === 'admin' ? '管理者' : profile.position ?? 'スタッフ'}</p>
      </div>
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive ? 'bg-amber-400/10 text-amber-300' : 'text-muted-foreground hover:bg-muted hover:text-foreground'
              )}
            >
              <Icon size={18} />
              {label}
